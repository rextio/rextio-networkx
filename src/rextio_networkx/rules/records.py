...
_RULES = (
    ...
    _native(
        "rextio-networkx/single-source-shortest-path-lengths",
        "single_source_shortest_path_lengths(GraphI64, NodeI64)",
        "Custom ordered BFS over the resident petgraph matches nx.single_source_shortest_path_length(G, source): source-first integer distance dict insertion and preserved neighbor discovery order.",
        "RXTP-NETWORKX-006",
        "Use GraphI64 and NodeI64 annotations with no options or keywords.",
    ),
    _native(
    ...
    _native(
        "rextio-networkx/shortest-path-length",
        "shortest_path_length(GraphI64, NodeI64, NodeI64)",
        "Resident BFS returns the exact unweighted source-target integer distance and preserves NetworkX 3.5 source-before-target NodeNotFound and NetworkXNoPath behavior.",
        "RXTP-NETWORKX-008",
        "Use GraphI64 and two NodeI64 annotations with no options or keywords.",
    ),
    _native(
        "rextio-networkx/number-of-nodes",
        "number_of_nodes(GraphI64)",
        "A checked scalar query borrows the resident unweighted graph and matches Graph.number_of_nodes(), including zero for the empty edge-induced graph.",
        "RXTP-NETWORKX-012",
        "Pass a GraphI64 produced by graph_from_edgelist.",
    ),
    _native(
        "rextio-networkx/number-of-edges",
        "number_of_edges(GraphI64)",
        "A checked scalar query borrows the resident graph and matches Graph.number_of_edges(), with duplicate/reversed edges deduplicated and each self-loop counted once.",
        "RXTP-NETWORKX-013",
        "Pass a GraphI64 produced by graph_from_edgelist.",
    ),
    _native(
    ...
    RuleRecord(
        ...
        constraint="Directed/multigraph/object-label calls, dynamic options, depth/cutoff forms, and other NetworkX APIs are not claimed.",
        outcome="fallback",
